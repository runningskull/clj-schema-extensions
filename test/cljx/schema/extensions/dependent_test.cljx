...
(deftest field-dependent-test
  (let [schema (field-dependent :type {:foo s/Str :bar s/Int}
                 {:t1 {:arg1 s/Str}
                  :t2 {:arg2 s/Str}
                  :t3 {:arg1 s/Str :barg s/Int}})
        schema2 (field-dependent :type {:t1 {:arg s/Str} :t2 {:arg s/Int}})]
    (testing "checks clean"
      ...
      (is (nil? (s/check schema2 {:type :t1 :arg "arg"})))
      (is (nil? (s/check schema2 {:type :t2 :arg 874 }))))
    (testing "invalid checks"
      ...
      (is (some? (s/check schema2 {:type :t2 :arg "arg" }))))))
...
(deftest schema-walker-field-dependent-test
  (testing "Walk a schema with field-dependent in it"
    (let [calls (atom [])
          schema {:a (field-dependent :type {:t1 {:x s/Str} :t2 {:y s/Int}})}]
